...
-- LEFT(문자열, 숫자)
-- 문자열을 왼쪽부터 숫자길이 만큼 잘라서 반환
SELECT LEFT('abcdef', 2);

-- RIGHT(문자열, 숫자)
-- 문자열을 오른쪽부터 숫자길이 만큼 잘라서 반환
...
-- 반향은 LEADING(좌), BOTH(좌우), TRAILING(우) 지정 가능
SELECT
	TRIM(LEADING'Bearer' FROM 'Bearer 34jQ3JLR');
...
-- 왼쪽부터 구분자가 횟수번쨰가 나오면 그 이후 문자열을 버림
-- 횟수를 음수로 절정시 오른쪽부터 적용
SELECT
	SUBSTRING_INDEX('차지영_그린_테스트', '_', -2);
...
SELECT TRUNCATE(3.151592, 2);

-- -------------------------
-- 날짜 및 시간 함수
-- -------------------------
-- NOW() : 현재 날짜 및 시간을 반환(YYY-MM-DD HH:MM:SS)
SELECT NOW();

-- DATE(데이트 타입의 값) : 데이트타입이 값을(YYY-MM-DD) 형식으로 변환
SELECT DATE(NOW());
...
-- COUNT(*) : 검색 결과의 총 레코드 수를 출력(NULL포함)
-- COUNT(컬럼명) : 검색 결과의 총 레코드 수를 출력(NULL비포함)
SELECT
	COUNT(*)
	,COUNT(end_at)
FROM salaries;
...
-- ROW_NUMBER() OVER(ORDER BY 컬럼ADC/DESC)
-- 레코드에 순위를 매겨 반환
-- 동일한 값이 있는 경우에도 각 행에 고유한 랭므를 부여
SELECT
	emp_id
	,salary
